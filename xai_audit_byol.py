"""Grad-CAM audit entrypoint for the BYOL-initialized Swin model."""
...
DEFAULT_MODEL_PATH = "./Pretraining/byol_swin_backbone.pth"
DEFAULT_DATA_DIR = "./Dataset of Tuberculosis Chest X-rays Images"
DEFAULT_OUTPUT_DIR = "./gradcam_outputs/byol"


def resolve_target_layer(model):
    backbone = model[0]
    return backbone.layers[2].blocks[-1].norm2


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Grad-CAM on the BYOL initialized Swin Transformer.")
    parser.add_argument("--model-path", default=DEFAULT_MODEL_PATH, help="Path to the fine-tuned BYOL checkpoint (.pth).")
...
def main() -> None:
    ...
    reshape_transform = make_swin_reshape_transform(stage_height=14, stage_width=14)
